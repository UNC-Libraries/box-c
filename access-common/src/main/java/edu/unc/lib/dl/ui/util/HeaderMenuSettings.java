...
public class HeaderMenuSettings {
	...
	public static Pattern entryMatcher = Pattern.compile("^menu\\.(\\w+)(\\.(label|url|entry|order|referer|target))?(\\.(\\w+).(label|url|referer))?$");
	...
	public static class HeaderMenu {
		...
		@Override
		public String toString() {
			return "HeaderMenu [subMenus=" + subMenus + ", key=" + key + ", label=" + label + ", url=" + url + ", order="
					+ order + "]";
		}
	}
}
