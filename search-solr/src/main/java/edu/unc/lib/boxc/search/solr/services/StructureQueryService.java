...
public class StructureQueryService extends AbstractQueryService {
    ...
    private Set<String> getDirectContainerMatches(SearchState baseState, AccessGroupSet accessGroups)
        ...
        directMatchState.setResultFields(asList(SearchFieldKey.ID.name()));
        ...
        String idField = solrField(SearchFieldKey.ID);
    ...
    private CutoffFacet getObjectPath(PID pid, AccessGroupSet accessGroups) {
        List<String> resultFields = asList(SearchFieldKey.ID.name(), ANCESTOR_PATH.name());
