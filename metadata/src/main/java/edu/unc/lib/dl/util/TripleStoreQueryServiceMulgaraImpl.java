...
public class TripleStoreQueryServiceMulgaraImpl implements
	...
	public Set<String[]> lookupGroupRoles(PID pid) {
		...
		String q = String.format(query.toString(),
				...
				ContentModelHelper.UserRole.patron.getURI(),
				ContentModelHelper.UserRole.observer.getURI(),
				ContentModelHelper.UserRole.ingester.getURI(),
				ContentModelHelper.UserRole.processor.getURI(),
				ContentModelHelper.UserRole.curator.getURI());
		List<List<String>> response = this.lookupStrings(q);
		if (!response.isEmpty()) {
			for (List<String> solution : response) {
