...
public class MetsDirDiff {
	...
	System.out
			.println("Usage: java -cp cdla-common.jar edu.unc.lib.dl.util.MetsDirDiff [options...] <mets file.xml> <directory path>");
	System.out.println(" <mets file.xml>\tfull path to a mets.xml file");
	System.out
			.println(" <directory path>\tfull path to the directory (within which METS file pointers are relative)");
	System.out
			.println("Options:\tMETS file and directory arguments can be in any order.\n --nopath\t match filenames and total numbers only (for checking http references)");
	System.exit(0);
    }
    ...
    private static void addFolder(File folder, Set<String> index, boolean nopath) throws IOException {
	for (File f : folder.listFiles()) {
		...
		if (nopath) {
		    ...
		    index.add(f.getCanonicalPath().substring(canonicalPathTrim).replace('\\', '/'));
		}
	    }
	}
    }
