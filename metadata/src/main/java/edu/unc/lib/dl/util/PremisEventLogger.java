...
public class PremisEventLogger {
    ...
    public static final Namespace NS = Namespace
            .getNamespace(JDOMNamespaceUtil.PREMIS_V2_NS.getURI());
    ...
    /**
     * This is an enumeration of accepted event types. These should be part of a
     * controlled vocabulary of event types used in CDR premis.
     */
    public enum Type {
        CAPTURE("capture"), // The process whereby a repository actively obtains
                            // an object.
        COMPRESSION("compression"), // The process of coding data to save
                                    // storage space or transmission time.
        CREATION("creation"), // The act of creating a new object.
        DEACCESSION("deaccession"), // The process of removing an object from
                                    // the inventory of a repository.
        DECOMPRESSION("decompression"), // The process of reversing the effects
                                        // of compression.
        DECRYPTION("decryption"), // The process of converting encrypted data to
                                  // plaintext.
        DELETION("deletion"), // The process of removing an object from
                              // repository storage.
        DIGITAL_SIGNATURE_VALIDATION("digitalSignatureValidation"),
        // The process of determining that a decrypted digital signature matches
        // an expected value.
        // EXCEPTION("exception"), // Any unexpected event that changes the
        // object.
        FIXITY_CHECK("fixityCheck"), // The process of verifying that an object
                                     // has not been changed in a given period.
        INGESTION("ingestion"), // The process of adding objects to a
                                // preservation repository.
        MESSAGE_DIGEST_CALCULATION("messageDigestCalculation"),
        // The process by which a message digest ("hash") is created.
        MIGRATION("migration"), // A transformation of an object creating a
                                // version in a more contemporary format.
        NORMALIZATION("normalization"), // A transformation of an object
                                        // creating a version more conducive to
                                        // preservation.
        REPLICATION("replication"), // The process of creating a copy of an
                                    // object that is, bitwise, identical to the
                                    // original.
        VALIDATION("validation"), // The process of comparing and object with a
                                  // standard and noting compliance or
                                  // exceptions.
        VIRUS_CHECK("virusCheck"); // The process of scanning a file for
                                   // malicious programs.
     ...
     */
    public static Element addDetailedOutcome(Element event, String outcome,
            String detailNote, Element detailExtension) {
        // log the detail xml
        Element eventOutcomeInfo = new Element("eventOutcomeInformation", NS)
                .addContent(new Element("eventOutcome", NS).setText(outcome));
        ...
        if (detailNote != null || detailExtension != null) {
            ...
            if (detailNote != null) {
                eventOutcomeDetail.addContent(new Element(
                        "eventOutcomeDetailNote", NS).setText(detailNote));
            }
            if (detailExtension != null) {
                eventOutcomeDetail.addContent(new Element(
                        "eventOutcomeDetailExtension", NS)
                        .addContent(detailExtension));
            }
        }
        return event;
    }
    ...
    public Element appendLogEvents(PID pid, Element eventsElement) {
        ...
        List children = this.pid2EventList.get(pid).cloneContent();// .getChildren("event",
                                                                   // NS);
    ...
    public static Element getObjectElement(PID pid) {
        ...
        id.addContent(new Element("objectIdentifierType", NS)
                .setText(PID_ID_TYPE));
        id.addContent(new Element("objectIdentifierValue", NS).setText(pid
                .getPid()));
        result.addContent(id);
        return result;
    }

    public static Element addSoftwareAgent(Element event, String name,
            String versionNumber) {
        // add linked agent
        Element lai = new Element("linkingAgentIdentifier", NS);
        lai.addContent(new Element("linkingAgentIdentifierType", NS)
                .setText("Name"));
        lai.addContent(new Element("linkingAgentIdentifierValue", NS)
                .setText(name + " (" + versionNumber + ")"));
    ...
    public static Element addDateTime(Element event, Date dateTime) {
        event.addContent(new Element("eventDateTime", NS).setText(dateFormat
                .format(dateTime)));
        return event;
    }

    public static Element addLinkingAgentIdentifier(Element event, String type,
    ...
    public static Element addLinkingObjectIdentifier(Element event,
            String type, String value, String role) {
        Element loi = new Element("linkingObjectIdentifier", NS);

        if (type != null) {
            loi.addContent(new Element("linkingObjectIdentifierType", NS)
                    .setText(type));
        }

        if (value != null) {
            loi.addContent(new Element("linkingObjectIdentifierValue", NS)
                    .setText(value));
        }
    ...
    public Element logEvent(PID pid) {
        ...
        String uuid = String.format("urn:uuid:%1$s",
                java.util.UUID.randomUUID());
        event.addContent(new Element("eventIdentifier", NS).addContent(
                new Element("eventIdentifierType", NS).setText("URN"))
                .addContent(
    ...
    public Element logEvent(Type type, String message, PID pid) {
        ...
        lai.addContent(new Element("linkingAgentIdentifierType", NS)
                .setText(PID_ID_TYPE));
        lai.addContent(new Element("linkingAgentIdentifierValue", NS)
                .setText(this.defaultAgent));
        lai.addContent(new Element("linkingAgentRole", NS).setText("Initiator"));
        event.addContent(lai);

        // add linked object (this pid)
        // REMOVED LINK TO SUBJECT PID, it is assumed from main object
        // identifier in the premis document.
        // Element obj = new Element("linkingObjectIdentifier", NS);
        // obj.addContent(new Element("linkingObjectIdentifierType",
        // NS).setText(PID_TYPE));
        // obj.addContent(new Element("linkingObjectIdentifierValue",
        // NS).setText(pid.getPid()));
        // obj.addContent(new Element("linkingObjectRole",
        // NS).setText("Subject"));
    ...
    public Element logEvent(Type type, String message, PID pid,
        ...
        source.addContent(new Element("linkingObjectIdentifierType", NS)
                .setText(PID_ID_TYPE));
        source.addContent(new Element("linkingObjectIdentifierValue", NS)
                .setText(pid.getPid() + "/" + dataStream));
        source.addContent(new Element("linkingObjectRole", NS)
                .setText("Source Data"));
        event.addContent(source);
        return event;
    }

    public Element logDerivationEvent(Type type, String message, PID pid,
            String sourceDataStream, String destDataStream) {
        Element event = this.logEvent(type, message, pid);
        ...
        source.addContent(new Element("linkingObjectIdentifierType", NS)
                .setText(PID_ID_TYPE));
        source.addContent(new Element("linkingObjectIdentifierValue", NS)
                .setText(pid.getPid() + "/" + sourceDataStream));
        source.addContent(new Element("linkingObjectRole", NS)
                .setText("Source Data"));
        event.addContent(source);
        Element dest = new Element("linkingObjectIdentifier", NS);
        dest.addContent(new Element("linkingObjectIdentifierType", NS)
                .setText(PID_ID_TYPE));
        dest.addContent(new Element("linkingObjectIdentifierValue", NS)
                .setText(pid.getPid() + "/" + destDataStream));
        dest.addContent(new Element("linkingObjectRole", NS)
                .setText("Derived Data"));
    ...
    public Element logException(String message, Throwable e) {
        ...
        // add event outcome
        PremisEventLogger.addDetailedOutcome(event, e.getLocalizedMessage(),
                sw.toString(), detailExtension);
