...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" 
    ...
    <sch:let name="profileUrl" value="'DSpace METS SIP Profile 1.0'"/>
    ...
    <sch:pattern>
        ...
        <sch:rule context="m:mets">
            <sch:assert test="@PROFILE = $profileUrl">The mets MUST indicate the DSpace profile.</sch:assert>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern>
        ...
        <sch:rule context="m:metsHdr">
            ...
            <sch:assert test="m:agent[@ROLE = 'CREATOR' and @TYPE = 'INDIVIDUAL'] or m:agent[@ROLE = 'CUSTODIAN']">The metsHdr MUST have at least one agent with ROLE of CREATOR and TYPE INDIVIDUAL.</sch:assert>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern>
        ...
        <sch:rule context="m:dmdSec">
            <sch:assert test="m:mdWrap[@MDTYPE='MODS'] or m:mdRef[@LOCTYPE='URL' and @MDTYPE='MODS'] or m:mdWrap[@OTHERMDTYPE='EPDCX']">Any dmdSec MUST contain MODS, a link to MODS xml, or EPDCX.</sch:assert>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern>
        ...
        <sch:rule context="m:file">
            <sch:assert test="not(parent::m:fileGrp/@USE) or parent::m:fileGrp/@USE='CONTENT'">fileGrp elements USE attribute must be CONTENT or not exist.</sch:assert>
    ...
    <sch:pattern>
        ...
        <sch:rule context="m:structMap">
            <sch:assert test="not(@TYPE) or @TYPE='Basic' or @TYPE='LOGICAL'">The specified structMap TYPE (<sch:value-of select="@TYPE"/>) MUST be blank or 'Basic'</sch:assert>
        </sch:rule>        
        ...
        <sch:rule context="m:div">
            <sch:assert test="not(@TYPE) or contains('Bag,Collection,Disk,Folder,AlphabetizedFolder,File,Reference,SWORD Object', @TYPE)">The specified TYPE (<sch:value-of select="@TYPE"/>) MUST be one of Disk, Folder, AlphabetizedFolder, File, Reference, or SWORD Object.</sch:assert>
        </sch:rule>
