...
public interface DigitalObjectManager {
     ...
     */
    public void addRelationship(PID subject, ContentModelHelper.Relationship rel, PID object) throws NotFoundException, IngestException;
    ...
    /**
     * Moves the specified objects from their current containers to another existing
     * container. The destination path must correspond to a object having the
     * Container model.  Note that the List may include PIDs from a variety of different
     * containers.  The items in the List are inserted among children in the new parent
     * according to their index within the List, unless some other sort is specified on the parent.
     *
     * @param movingPids
