...
/**
 * Looks for objects in the repository with matching repository paths. Conflict
 * information is then passed to a merge strategy class that performs required
 * merging logic or throws a MergeException.
 *
 * @author count0
 *
 */
public class CheckPathConflictFilter implements AIPIngestFilter {
	...
	do {
	    ...
	    slugs.push(slug);
	    top = step;
	    step = JRDFGraphUtil.getPIDRelationshipSubject(rdfaip.getGraph(), ContentModelHelper.Relationship.contains
			    .getURI(), step);
	} while (step != null);
