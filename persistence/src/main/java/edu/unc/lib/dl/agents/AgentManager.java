...
public class AgentManager {
	...
	SoftwareAgent result = new SoftwareAgent(ContentModelHelper.Administrative_PID.REPOSITORY_MANAGEMENT_SOFTWARE.getPID(),
	...
	List<PID> resp = this.getDigitalObjectManager().add(sip, user, message, false);
	log.debug("response is has this many PIDs:" + resp.size());
	log.debug("response[0] is:" + resp.get(0));
	return this.getGroupAgents(resp, false).get(0);
    }
     ...
     */
    public void addMembership(GroupAgent group, PersonAgent member, Agent user) throws NotFoundException, IngestException {
	...
	if(conflict != null) {
	    throw new IngestException("A person with the Onyen '" + person.getOnyen() + "' already exists: " + conflict.getOnyen() +", "+ conflict.getPID());
	}
	...
	log.debug("response is has this many PIDs:" + resp.size());
	log.debug("response[0] is:" + resp.get(0));
	return this.getPersonAgents(resp, false).get(0);
    }
	...
	log.debug("response is has this many PIDs:" + resp.size());
	log.debug("response[0] is:" + resp.get(0));
	return this.getSoftwareAgents(resp).get(0);
    }
	...
	String query = String.format(q.toString(), this.getTripleStoreQueryService().getResourceIndexModelUri(),
			...
			ContentModelHelper.FedoraProperty.hasModel, ContentModelHelper.Model.GROUPAGENT);
	...
	String query = String.format(q.toString(), this.getTripleStoreQueryService().getResourceIndexModelUri(),
			ContentModelHelper.FedoraProperty.label, ContentModelHelper.CDRProperty.onyen, ContentModelHelper.FedoraProperty.hasModel,
			ContentModelHelper.Model.PERSONAGENT);
	List<List<String>> resp = this.getTripleStoreQueryService().queryResourceIndex(query);
	...
	String query = String.format(q.toString(), this.getTripleStoreQueryService().getResourceIndexModelUri(),
			ContentModelHelper.FedoraProperty.label,
			ContentModelHelper.FedoraProperty.hasModel, ContentModelHelper.Model.SOFTWAREAGENT);
	List<List<String>> resp = this.getTripleStoreQueryService().queryResourceIndex(query);
     ...
     *             if either object are not found in the repository
     */
    public void removeMembership(GroupAgent group, PersonAgent member, Agent user) throws NotFoundException, IngestException {
