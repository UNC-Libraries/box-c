...
public class TripleStoreManagerMulgaraImpl implements TripleStoreManager {
	...
	/*
	 * try { this.tempKruleOwl = File.createTempFile("krule-", ".owl");
	 * this.tempKruleRdf = File.createTempFile("krule-", ".rdf"); // dump
	 * rules files into temporary space for ITQL loading BufferedInputStream
	 * isOwl = new
	 * BufferedInputStream(this.getClass().getClassLoader().getResourceAsStream
	 * ( TripleStoreQueryServiceMulgaraImpl.kruleOwlPath));
	 * BufferedOutputStream osOwl = new BufferedOutputStream(new
	 * FileOutputStream(this.tempKruleOwl)); byte[] buf = new byte[1024];
	 * int len; while ((len = isOwl.read(buf)) > 0) { osOwl.write(buf, 0,
	 * len); } isOwl.close(); osOwl.close();
	 *
	 * BufferedInputStream isRdf = new
	 * BufferedInputStream(this.getClass().getClassLoader
	 * ().getResourceAsStream(
	 * TripleStoreQueryServiceMulgaraImpl.kruleRdfPath));
	 * BufferedOutputStream osRdf = new BufferedOutputStream(new
	 * FileOutputStream(this.tempKruleRdf)); while ((len = isRdf.read(buf))
	 * > 0) { osRdf.write(buf, 0, len); } isRdf.close(); osRdf.close(); }
	 * catch (IOException e) { throw new Error("There was a problem
	 * initializing the Triple Store Service", e); }
	 */
    }
    ...
    /**
     * Sets up the Mulgara triple store to process inferences based on the
     * Fedora external relationship ontology.
     */
    ...
    /**
     * Gets any Mulgara service ready to house repository statements from
     * scratch. Sets up an empty model for the resource index Sets up the rules
     * and schema models for entailment. Sets up a target model for entailed
     * statements.
     */
    ...
    /**
     * This method will cause the resource index model to be reloaded from
     * RELS-EXT datastreams. Digital Object updates
     */
     ...
     *
     * @see
     * edu.unc.lib.dl.services.TripleStoreService#updateCoreTransactionalInferences
     * ()
     */
	...
	this.sendTQL(String.format("apply <%1$s> to <%2$s> <%3$s>;", this.getInferenceRulesModelUri(), this
			.getResourceIndexModelUri(), this.getInferredModelUri()));
    }
