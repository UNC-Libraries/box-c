...
public class FolderManager {
    ...
    /**
     * Given a repository path string, creates any folders that do not yet exist
     * along this path.
     *
     * @param path
	...
	for (int i = slugs.size(); i >= 0; i--) {
		...
		if (!types.contains(ContentModelHelper.Model.CONTAINER.getURI())) {
		    ...
		    throw new IngestException("The object at the path '" + test + "' is not a folder.  It has these content models:\n"
				    + s.toString());
		}
		containerPath = test;
		break;
	...
	while (createFolders.size() > 0) {
	    ...
	    lastpid = this.getDigitalObjectManager().add(sip, user, "creating a new folder path", false).get(0);
	}
	return lastpid;
    }
