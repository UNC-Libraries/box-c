...
public class METSPackageSIPProcessor implements SIPProcessor {
	...
	try {
		...
		public Source resolve(String href, String base) throws TransformerException {
		    Source result = null;
		    result = new StreamSource(METSPackageSIPProcessor.class.getResourceAsStream(stylesheetPackage
				    + href));
		    return result;
		}
	    });
	...
	for(PID p : rdfaip.getPIDs()) {
	    String inslug = JRDFGraphUtil.getRelatedLiteralObject(rdfaip.getGraph(), p, ContentModelHelper.CDRProperty.slug.getURI());
	    String slug = inslug;
	    while(usedSlugs.contains(slug)) {
		slug = PathUtil.incrementSlug(slug);
	    }
	    if(!inslug.equals(slug)) {
		JRDFGraphUtil.removeAllRelatedByPredicate(rdfaip.getGraph(), p, ContentModelHelper.CDRProperty.slug.getURI());
    ...
    private AIPImpl transformMETS(PremisEventLogger logger, METSPackageSIP metsPack, Document mets,
