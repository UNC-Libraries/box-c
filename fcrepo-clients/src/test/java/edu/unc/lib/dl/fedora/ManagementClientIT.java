...
/**
 * This class tests exception conditions that should arise when Fedora is
 * offline or the properties of the client are misconfigured to make it
 * unreachable.
 * 
...
public class ManagementClientIT {
	...
	private ManagementClient getManagementClient() {
		return this.beanFactory.getBean("managementClient",
	...
	public void testIngestDuplicatePID() throws Exception {
		...
		try {
			c.purgeObject(new PID("cdr-test:Container"), "testing ingest",
					false);
		...
		byte[] testData = null;
		try (InputStream xmlis = new FileInputStream(testFile)) {
