 ...
 */
package edu.unc.lib.cdr.indexing.triplestore;
...
public class TriplestoreRouter extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		final Namespaces ns = new Namespaces("rdf",
				"http://www.w3.org/1999/02/22-rdf-syntax-ns#");
		ns.add("indexing", "http://fedora.info/definitions/v4/indexing#");

		final XPathBuilder indexable = new XPathBuilder(String.format(
				"/rdf:RDF/rdf:Description/rdf:type[@rdf:resource='%s']",
		...
		from("{{input.stream}}")
			...
			.to("direct:delete.triplestore").otherwise()
			.to("direct:index.triplestore");

		/**
		 * Handle re-index events
		 */
		from("{{triplestore.reindex.stream}}").routeId(
			"FcrepoTriplestoreReindex").to("direct:index.triplestore");
		...
		from("direct:index.triplestore")
			...
			.filter(not(in(tokenizePropertyPlaceholder(getContext(),
				"{{filter.containers}}", ",")
			.stream()
			.map(uri -> or(
			...
			.to("direct:update.triplestore").otherwise()
