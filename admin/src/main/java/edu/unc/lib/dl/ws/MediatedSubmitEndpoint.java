...
@Endpoint
public class MediatedSubmitEndpoint extends WebServiceGatewaySupport {
	private final Logger logger = Logger.getLogger(getClass());

	private SubmitService submitService;

	@PayloadRoot(localPart = Constants.MEDIATED_SUBMIT_INGEST_OBJECT, namespace = Constants.NAMESPACE)
	public MediatedSubmitIngestObject mediatedSubmit(
			MediatedSubmitIngestObject request) {

		return submitService.mediatedSubmit(request);
