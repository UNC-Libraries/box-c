...
@Controller
public class AddToSearchController {
	...
	if(logger.isDebugEnabled()) logger.debug("requestURL " + request.getRequestURL());
	if(logger.isDebugEnabled()) logger.debug("requestURI " + request.getRequestURI());
	if(logger.isDebugEnabled()) logger.debug("querystring " + request.getQueryString());
	...
	for (int i = 0; i < keys.length; i++) {
		...
		if(logger.isDebugEnabled()) logger.debug(i + " " + j + " key: " + keys[i] + " value: "
			+ values[j]);
	    }
	}

	uiWebService.addToSearchFromIrUrlInfo(irUrlInfo, "test");
    }
