...
@Endpoint
public class GetFromXmlDbEndpoint extends WebServiceGatewaySupport {
	private final Logger logger = Logger.getLogger(getClass());
	private XmlDbService xmlDbService;

	@PayloadRoot(localPart = Constants.GET_FROM_XML_DATABASE_REQUEST, namespace = Constants.NAMESPACE)
	public GetFromXmlDbResponse GetFromXmlDb(GetFromXmlDbRequest getRequest) {
		GetFromXmlDbResponse response = new GetFromXmlDbResponse();
		...
		response.setResponse(xmlDbService.getFromXmlDb(query));
		return response;
	}
