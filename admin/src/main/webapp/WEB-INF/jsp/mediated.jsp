...
<title><fmt:message key="submit.mediated.heading" /></title>
...
<script type="text/javascript">
	$(document).ready( function() {
		...
		$("#checksumDate").datepicker({dateFormat : 'yy-mm-dd'}).val($.datepicker.formatDate('yy-mm-dd', new Date()));
		$("#checksumDate").datepicker('option', 'constrainInput', true);
		$("#checksumDate").datepicker('option', 'maxDate', '+0m +0w');
	});
</script>
</head>
<body>
<div class="records_hdr">
<h1><fmt:message key="submit.mediated.heading" /></h1>
</div>
<form:form method="POST" commandName="mediatedSubmitDAO"
	enctype="multipart/form-data">
	<c:out value="${mediatedSubmitDAO.message}" />
	<table width="50%" bgcolor="f8f8ff" border="0" cellspacing="0"
		...
		<tr>
			<td align="left"><fmt:message key="submit.submissionCheck" /></td>
			<td align="left"><spring:bind
				path="mediatedSubmitDAO.submissionCheck">
				<input type="hidden" name="_<c:out value="${status.expression}"/>"
					value="visible" />
				<input type="checkbox" name="<c:out value="${status.expression}"/>"
					value="true" <c:if test="${status.value}">checked</c:if>>
			</spring:bind></td>
			<td><spring:bind path="mediatedSubmitDAO.submissionCheck">
				<FONT color="red"><B><c:out
					value="${status.errorMessage}" /></B></FONT>
			</spring:bind></td>
		</tr>
		...
		<tr>
			<td align="left"><fmt:message key="submit.filepath" /></td>
			<td align="left"><input type="text" name="filePath"
				value="${mediatedSubmitDAO.filePath}" size="60"></td>
		...
		<tr>
			<td align="left"><fmt:message key="submit.file" /></td>
		...
		<tr>
			<td align="left"><fmt:message key="submit.checksum" /></td>
			<td align="left"><input type="text" name="checksum"
				value="${mediatedSubmitDAO.checksum}" size="60"></td>
		</tr>
		<tr>
			<td align="left"><fmt:message key="submit.checksumDate" /></td>
			<td align="left"><input type="text" id="checksumDate"
				name="checksumDate" value="${mediatedSubmitDAO.checksumDate}"
				size="10" readonly="readonly"></td>
			<td><spring:bind path="mediatedSubmitDAO.checksumDate">
				<FONT color="red"><B><c:out
					value="${status.errorMessage}" /></B></FONT>
			</spring:bind></td>
		</tr>
		<tr>
			<td align="left"><fmt:message key="submit.metadata" /></td>
			<td align="left"><input type="file" name="metadata"
				value="${mediatedSubmitDAO.metadata}" size="60" /></td>
			<td><spring:bind path="mediatedSubmitDAO.metadata">
				<FONT color="red"><B><c:out
					value="${status.errorMessage}" /></B></FONT>
			</spring:bind></td>
		</tr>
