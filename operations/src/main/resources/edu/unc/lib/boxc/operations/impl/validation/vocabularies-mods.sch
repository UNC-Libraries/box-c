...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron"
	...
	<sch:let name="path" value="/edu/unc/lib/boxc/operations/impl"/>
	
	<sch:let name="langISO"
		value="document('/edu/unc/lib/boxc/operations/impl/validation/ISO-639-2_utf-8.xml')//value"/>
	
	<!-- commenting out genres -->
	<!--<sch:let name="genreTGM" value="document('/edu/unc/lib/boxc/operations/impl/validation/tgm2.xml')//DESCRIPTOR"/>
	<sch:let name="genreEPRINTS"
		value="document('/edu/unc/lib/boxc/operations/impl/validation/genres_eprints_swap.xml')//value"/>
	<sch:let name="genreEPRINTSURI"
		value="document('/edu/unc/lib/boxc/operations/impl/validation/genres_eprints_swap.xml')//valueURI"/>
	-->
	...
	<sch:pattern>
		...
		<sch:rule context="mods:originInfo/mods:*[@keyDate='yes']">
			<sch:assert test="@encoding = 'iso8601' or @encoding = 'edtf' or @encoding = 'w3cdtf'">Dates must be explicitly encoded according to
				the ISO 8601, edtf, or W3CDTF standards. (The encoding attribute must be 'iso8601', 'edtf', or 'w3cdtf'.)</sch:assert>
		</sch:rule>
	</sch:pattern>
	...
	<sch:pattern>
		...
		<sch:rule context="mods:identifier[@displayLabel='Collection Number' and @type='local']">
			<sch:assert test="matches(.,'^[0-9a-zA-Z_\\-]{5}[0-9a-zA-Z_\\-]*$')">Collection Number must contain only letters, numbers or the - character, and be at least 5 characters long.</sch:assert>
		</sch:rule>
	</sch:pattern>
