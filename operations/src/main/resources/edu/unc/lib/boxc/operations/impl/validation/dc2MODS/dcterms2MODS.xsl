...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	exclude-result-prefixes="dcterms" version="2.0">
    <xsl:output method="xml" indent="yes" encoding="UTF-8"/>
    <xsl:include href="dc2MODS/dcmiType.xsl"/>
    <xsl:include href="dc2MODS/mimeType.xsl"/>
    <xsl:include href="dc2MODS/csdgm.xsl"/>
    <xsl:include href="dc2MODS/forms.xsl"/>
    <xsl:include href="dc2MODS/iso3166-1.xsl"/>
    <xsl:include href="dc2MODS/iso639-2.xsl"/>
    ...
    <xsl:template match="/">
        <xsl:if test="dcterms:dc">
            <xsl:apply-templates/>
        </xsl:if>
    </xsl:template>
    <xsl:template match="dcterms:dc">
    ...
    <xsl:template name="dcMain">
	    <xsl:for-each select="dcterms:title">
            ...
            <xsl:for-each select="dcterms:subject | dcterms:coverage">
                <xsl:apply-templates select="."/>
            </xsl:for-each>
            ...
            <xsl:for-each select="dcterms:source | dcterms:relation">
                <xsl:apply-templates select="."/>
            </xsl:for-each>
	...
	<xsl:choose>
            <xsl:when test="string(text()) = 'Dataset' or string(text()) = 'dataset'">
                <typeOfResource>
			<xsl:if test="../dcterms:type[string(text()) = 'collection' or string(text()) = 'Collection']">
