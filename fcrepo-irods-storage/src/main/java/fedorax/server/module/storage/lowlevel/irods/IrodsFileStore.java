...
public class IrodsFileStore {
    ...
    public IrodsFileStore(PathAlgorithm pathAlgorithm, IrodsDBPathRegistry pathRegistry, IrodsIFileSystem fileSystem)
	...
	if (filePath == null || filePath.equals("")) { // guard against
	    // algorithm
	    // implementation
	    LowlevelStorageException nullPath = new LowlevelStorageException(true, "null path from algorithm for pid "
			    + pid);
	    throw nullPath;
	}
	...
	try {
	    ...
	    LowlevelStorageException newFile = new LowlevelStorageException(true, "couldn't make File for " + filePath,
			    eFile);
	    throw newFile;
	}
	...
	try {
	    ...
	    LowlevelStorageException noPath = new LowlevelStorageException(false, "pid " + pid + " not in registry",
			    ffff);
	    throw noPath;
	}
	...
	try {
	    ...
	    LowlevelStorageException newFile = new LowlevelStorageException(true, "couldn't make new File for "
			    + filePath, eFile);
	    throw newFile;
	}
	return fileSystem.rewrite(file, content);
    }
	...
	if (filePath == null || filePath.equals("")) { // guard against registry
						       // implementation
	    LowlevelStorageException nullPath = new LowlevelStorageException(true, "null path from registry for pid "
			    + pid);
	    throw nullPath;
	}
    ...
    public IRODSQueryResultSet getMetadata(String pid) throws LowlevelStorageException {
	String filePath;
	filePath = pathRegistry.get(pid);
