...
/**
 * This class is an adapter to bridge between the Fedora default lowlevel
 * storage file system interface and the iRODS Jargon API. This adapter
 * guarantees faithful transport for all write operations to iRODS via MD5
 * checksum comparison. Any failed checksum comparison results in delete of the
 * file in question from iRODS and throwing a LowlevelStorageException. The
 * class uses Java I/O buffered streams to prevent caller code from blocking
 * when possible. In addition to the standard Fedora lowlevel storage
 * operations, this class has a getMetadata operation, which queries iRODS for a
 * standard set of system metadata.
 *
...
public class IrodsIFileSystem {
    ...
    /*
     * protected IRODSFileSystem getFileSystem() throws IOException { // GJ - no
     * connection reuse to facilitate recovery from broken socket // if
     * (reuseConnections && conn != null && conn.isConnected() && //
     * !conn.isClosed()) { // currentConnectionUsage++; // return conn; // }
     * LOG.info("Getting iRODS connection #" + (connectionsUsed + 1) +
     * ". Last connection used " + currentConnectionUsage + " times"); Exception
     * thrown = null; for (int attempts = 1; attempts <=
     * this.maxConnectAttempts; attempts++) { try { conn = (IRODSFileSystem)
     * FileFactory.newFileSystem(irodsAccount);
     * LOG.info("Created iRODS connection #" + connectionsUsed + 1);
     * currentConnectionUsage = 1; connectionsUsed++; return conn; } catch
     * (Exception e) { LOG.error("Failed to connect to iRODS, attempt " +
     * attempts + " of " + this.maxConnectAttempts, e); thrown = e; } int[]
     * waitSeconds = { 1, 5, 10, 20, 30 }; int sleep = 60; try { sleep =
     * waitSeconds[attempts]; } catch (Exception ignored) { } try {
     * LOG.info("Sleeping for " + sleep +
     * " seconds before next iRODS connect attempt."); Thread.sleep(sleep *
     * 1000); } catch (InterruptedException ignored) { } } throw new
    ...
    /*
     * public static class ReadTrapBufferedInputStream extends InputStream {
     * private BufferedInputStream buffered = null; private IRODSFileSystem
     * fileSystem = null;
     *
     * public ReadTrapBufferedInputStream(IRODSFileInputStream in, int
     * bufferSize, IRODSFileSystem fs) { this.buffered = new
     * BufferedInputStream(in, bufferSize); this.fileSystem = fs; }
     *
     * @Override public synchronized int read() throws IOException { try {
     * return buffered.read(); } catch (IOException e) {
     * this.fileSystem.close();
     * LOG.error("Closed fileSystem due to IOException: " + e.getMessage());
     * throw e; } }
     *
     * @Override public int read(byte[] b) throws IOException { try { return
     * super.read(b); } catch (IOException e) { this.fileSystem.close();
     * LOG.error("Closed fileSystem due to IOException: " + e.getMessage());
     * throw e; } }
     *
     * @Override public synchronized int read(byte[] b, int off, int len) throws
     * IOException { if (len == 0) { return 0; } try { return buffered.read(b,
     * off, len); } catch (IOException e) { this.fileSystem.close();
     * LOG.error("Closed fileSystem due to IOException: " + e.getMessage());
     * throw e; } }
     *
     ...
     * buffered.close(); }
     *
     * @Override public int available() throws IOException { return
     * buffered.available(); }
     *
     * }
     */
	...
	try {
	    ...
	    IRODSFile destination = irodsFileSystem.getIRODSFileFactory(account).instanceIRODSFile(
	    ...
	    IRODSFile trueLocation = irodsFileSystem.getIRODSFileFactory(account).instanceIRODSFile(
			    file.getAbsolutePath());
	    ...
	    IRODSFileOutputStream out = irodsFileSystem.getIRODSFileFactory(account)
			    .instanceIRODSFileOutputStream(temp);
	...
	metadataFields = new RodsGenQueryEnum[] { RodsGenQueryEnum.COL_DATA_SIZE, RodsGenQueryEnum.COL_D_MODIFY_TIME,
			...
			RodsGenQueryEnum.COL_D_DATA_CHECKSUM, RodsGenQueryEnum.COL_DATA_VERSION,
			RodsGenQueryEnum.COL_DATA_NAME, RodsGenQueryEnum.COL_COLL_NAME,
			RodsGenQueryEnum.COL_DATA_REPL_NUM, RodsGenQueryEnum.COL_D_REPL_STATUS,
			RodsGenQueryEnum.COL_D_RESC_NAME, RodsGenQueryEnum.COL_R_LOC,
			RodsGenQueryEnum.COL_R_CLASS_NAME, RodsGenQueryEnum.COL_R_RESC_COMMENT,
			RodsGenQueryEnum.COL_R_RESC_INFO, RodsGenQueryEnum.COL_R_TYPE_NAME,
