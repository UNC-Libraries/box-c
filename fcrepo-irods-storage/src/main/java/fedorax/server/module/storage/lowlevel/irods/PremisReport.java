 ...
 * information which is general to the Fedora object model or iRODS system metadata.  Output from this report
 * builder may be merged with other PREMIS sources to create more comprehensive reports.  This report includes
 * object identifier types that are Fedora object PIDs and the less familiar Fedora datastream version PIDs.  These
 * PIDs can be used to merge reports and navigate the object relationships within them.
 *
...
public class PremisReport {
    private static final Log LOG = LogFactory.getLog(PremisReport.class);
	...
	manager =
            (DOManager) fcserver.getModule("org.fcrepo.server.storage.DOManager");
	if (manager == null) {
	    throw new Exception("Can't get a DOManager "
	...
	object
		.addContent(new Element("objectIdentifier",p)
			.addContent(new Element("objectIdentifierType",p).setText("Fedora PID"))
			.addContent(new Element("objectIdentifierValue",p).setText(obj.getPid())));
        ...
        while( dsids.hasNext() ) {
    		...
    		object.addContent(new Element("relationship",p)
    			...
    			.addContent(new Element("relatedObjectIdentification",p)
    				...
    				.addContent(new Element("relatedObjectIdentifierValue",p).setText(dsVID))));
        }
	...
	object
		.addContent(new Element("objectIdentifier",p)
			.addContent(new Element("objectIdentifierType",p).setText("Fedora Datastream PID"))
			.addContent(new Element("objectIdentifierValue",p).setText(dsPID)))
		.addContent(new Element("objectCharacteristics",p)
			.addContent(new Element("compositionLevel",p).setText("1"))
			.addContent(new Element("size",p).setText(Integer.toString((int) size)))
			.addContent(new Element("format",p)
				.addContent(new Element("formatDesignation",p)
					.addContent(new Element("formatName",p).setText(ds.DSMIME)))
					.addContent(new Element("formatNote",p).setText("IANA MIME-type"))));
	...
	object
		...
		.addContent(new Element("objectCharacteristics",p)
			...
			.addContent(new Element("fixity",p)
			...
			.addContent(new Element("format",p)
				.addContent(new Element("formatDesignation",p)
					.addContent(new Element("formatName",p).setText(ds.DSMIME)))
					.addContent(new Element("formatNote",p).setText(formatNote))));
...
//	Resource Zone Name: cdrZone
	...
	String port = String.valueOf(this.irodslls.account.getPort());
	...
	object.addContent(new Element("storage",p)
		.addContent(new Element("contentLocation", p)
			.addContent(new Element("contentLocationType",p).setText("iRODS URI"))
			.addContent(new Element("contentLocationValue",p).setText(contentLocURI)))
		.addContent(new Element("storageMedium", p).setText(storageMedium)));
	} catch(JargonException e) {
	    throw new Error(e);
	}
    }
