...
/**
 * This class is used to record events that occur on the SIP prior to ingest. These events apply to all the objects in
 * the SIP equally. Individual object IDs are not recorded in this logger, but will be added later after PIDs are
 * assigned when events are moved to the AIP.
 * 
 * PID have not been assigned yet when these events are logged. This is a convenience class and various SIP
 * implementations may expose it to SIP creators or not.
 * 
 * SIP implementations may provide other means for more granular pre-ingest events to be passed to the AIP, for example
 * a metadata block in METS.
 * 
...
public class PreIngestEventLogger {
	...
	/**
	 * Add a MD5 check calculation event. (use the earliest checksum calculation available)
	 * 
	 * @param date
	...
	/**
	 * Add a SIP creation event, usually the act of creating a manifest. This is not the same as the ingest event.
	 * 
	 * @param date
