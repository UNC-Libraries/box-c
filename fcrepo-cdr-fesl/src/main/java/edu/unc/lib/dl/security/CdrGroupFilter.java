 ...
 */
public class CdrGroupFilter
        implements Filter {

    private static final Logger logger =
            LoggerFactory.getLogger(CdrGroupFilter.class);
    ...
    public void doFilter(ServletRequest request,
		...
		// CDR Specific
		String groups = req.getParameter("cdrRoles");

		if(groups == null) {
			
			groups = req.getHeader("cdrRoles");
			
			logger.debug("cdrRoles header: "+groups);
		}
		
		if (groups != null) {
			
			String[] cdrRoles = groups.split(";");
			
			if (cdrRoles != null) {
				for (String cdrRole : cdrRoles) {
					userRoles.add(cdrRole);
				}
			}
		}

		// userRoles.add("unc:app:lib:cdr:libtestgroup");
