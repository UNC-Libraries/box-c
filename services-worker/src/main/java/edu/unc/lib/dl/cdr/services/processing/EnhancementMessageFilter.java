...
public class EnhancementMessageFilter implements MessageFilter {
    ...
    public boolean filter(ActionMessage aMessage) {
        ...
        boolean applyServiceStack = JMSMessageUtil.ServicesActions.APPLY_SERVICE_STACK.equals(
        ...
        for (ObjectEnhancementService s : services) {
            try {
                ...
                if (serviceReached) {
                    //add services to the message's service list which have not failed previously
                    // and pass the prefilter method.
                ...
                LOG.error("Error while performing prefilterMessage on " + pid
                        + " for service " + s.getClass().getName(), e);
            }
        }
