        ...
        try {
            ...
            for (String srcURI : srcDSURIs) { // for each source datastream
                ...
                if (newestSourceDS != null) {
                    ...
                    dsLocation = newestSourceDS
                            .getChild("contentLocation", JDOMNamespaceUtil.FOXML_NS).getAttributeValue("REF");
            ...
            for (String dsid : ds2FitsDoc.keySet()) {
                ...
                // For example some pdfs come with the mime-type application/(x-)download
                // from fedora, which is unusable by browsers
                ...
                if ("DATA_FILE".equals(dsid)) {
                    ...
                    try {
                        ...
                        client.setExclusiveLiteral(pid, CDRProperty.hasSourceFileSize.getPredicate(),
                                CDRProperty.hasSourceFileSize.getNamespace(), size,
                ...
                p.addContent(new Element("object", PREMIS_V2_NS)
                        .addContent(
                                new Element("objectIdentifier", PREMIS_V2_NS).addContent(
                                        new Element("objectIdentifierType", PREMIS_V2_NS)
                        ...
                        .addContent(
                                objCharsEl
                                        ...
                                        .addContent(
                                                new Element("objectCharacteristicsExtension", PREMIS_V2_NS).addContent(
                                                        ds2FitsDoc.get(dsid).detachRootElement())))
        ...
        try {
            if (filename == null) {
                reader = new BufferedReader(new InputStreamReader(
                        ((AbstractIrodsObjectEnhancementService) service).remoteExecuteWithPhysicalLocation(
                                "fitsextract", dsIrodsPath)));
            } else {
                reader = new BufferedReader(new InputStreamReader(
                        ((AbstractIrodsObjectEnhancementService) service)
                        .remoteExecuteWithPhysicalLocation("fitsextract",
