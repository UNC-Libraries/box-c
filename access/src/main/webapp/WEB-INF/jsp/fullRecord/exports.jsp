...
<div class="contentarea">
	...
	<ul>
		<c:if test="${permsHelper.hasDatastreamAccess(requestScope.accessGroupSet, 'RELS-EXT', briefObject)}">
			<li><a href="${cdr:getDatastreamUrl(briefObject.id, 'RELS-EXT', fedoraUtil)}">Fedora Object-to-Object Relationships</a></li>
		</c:if>
		<c:if test="${permsHelper.hasDatastreamAccess(requestScope.accessGroupSet, 'techmd_fits', briefObject)}">
			<li><a href="${cdr:getDatastreamUrl(briefObject.id, 'techmd_fits', fedoraUtil)}">FITS Extract</a></li>
		</c:if>
		<c:if test="${permsHelper.hasDatastreamAccess(requestScope.accessGroupSet, 'MD_DESCRIPTIVE', briefObject)}">
			<li><a href="${cdr:getDatastreamUrl(briefObject.id, 'MD_DESCRIPTIVE', fedoraUtil)}">MODS</a></li>
		</c:if>
		<c:if test="${permsHelper.hasDatastreamAccess(requestScope.accessGroupSet, 'DC', briefObject)}">
			<li><a href="${cdr:getDatastreamUrl(briefObject.id, 'DC', fedoraUtil)}">OAI Dublin Core</a></li>
		</c:if>
		<c:if test="${permsHelper.hasDatastreamAccess(requestScope.accessGroupSet, 'MD_EVENTS', briefObject)}">
