...
public class SolrUpdateRunnable implements Runnable {
	...
	private void cleanReindex(SolrUpdateRequest updateRequest) {
		SolrUpdateRequest deleteTreeRequest = new SolrUpdateRequest(updateRequest.getPid(),
				SolrUpdateAction.DELETE_SOLR_TREE);
		SolrUpdateRequest commitRequest = new SolrUpdateRequest(updateRequest.getPid(), SolrUpdateAction.COMMIT);
		SolrUpdateRequest recursiveAddRequest = new SolrUpdateRequest(updateRequest.getPid(),
				SolrUpdateAction.RECURSIVE_ADD);
		...
		try {
			...
			if (resultDoc != null && resultDoc.getRootElement().getChild("digitalObject", foxmlNS) != null) {
				...
				if (relsExt != null && containerXpath.selectSingleNode(relsExt) != null) {
					...
					CountDownUpdateRequest commitRequest = new CountDownUpdateRequest(updateRequest.getPid(),
							SolrUpdateAction.COMMIT, cleanupRequest);
	...
	private void addObject(SolrUpdateRequest updateRequest) throws Exception {
		...
		if (resultDoc != null && resultDoc.getRootElement().getChild("digitalObject", foxmlNS) != null) {
			...
			// Issue update requests for all objects this item is the default web object for
			List<PID> defaultWebObjectSubjects = solrUpdateService
					.getFedoraDataService()
					.getTripleStoreQueryService()
					.fetchByPredicateAndLiteral(ContentModelHelper.CDRProperty.defaultWebObject.getURI().toString(),
							new PID(updateRequest.getPid()));
			if (defaultWebObjectSubjects != null) {
				LOG.debug(updateRequest.getPid() + " is the default web object for " + defaultWebObjectSubjects.size() + " objects.");
				for (PID subject: defaultWebObjectSubjects) {
					getSolrUpdateService().offer(subject.getPid());
				}
			} else {
				LOG.debug(updateRequest.getPid() + " is not the default web object for any objects.");
			}
		}
	}
					...
					if (solrUpdateService.getCollisionList() != null && !solrUpdateService.getCollisionList().isEmpty()) {
						...
						while (collisionIt.hasNext()) {
							...
							synchronized (solrUpdateService.getLockedPids()) {
								if (!solrUpdateService.getLockedPids().contains(updateRequest.getPid())
