...
public class Mp44uAudioProcessor implements Processor {
    ...
    public Mp44uAudioProcessor(String mp44uThreads, int mp44uTimeout) {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        if (exitCode != 0) {
            throw new Mp44uExecutionException("mp44u audio command " + Arrays.toString(command)
                    + " failed to execute for " + audioPath);
        }
    }
